    ...
    optionsForm = {
        ...
        createRow: {
            basic: function(element){
                ...
                $.each(element.attr, function(op, i){
                    if (DEBUG)
                        console.log(op, element.attr[op]);
                    $('#'+ element.id ).attr(op, element.attr[op]);
                });$.each(element.data, function(op, i){
                    if (DEBUG)
                        console.log(op, element.data[op]);
        ...
        setupForm:function(){
            ...
            for (; i<optionsForm.options.length; i++){
                ...
                var item = optionsForm.options[i],
                    ...
                    parent, defaultSectionClass, tag ;
                ...
                element.parent = parent = (element.parent) ? $('form fieldset.'+ element.parent) : $('form fieldset.settings__section--default');
                ...
                if (element.type === "button" || element.type === "checkbox" || element.type === "text" || element.type === "password" || element.type === "textarea"){
                    ...
                    tag = element.tag || "h2";
                    $(parent)
                        .append('<'+ tag +' id="'+ id +'">'+ element.label +'</'+tag+'>');
...
$(document).ready(function(){
    ...
    $(document).on('click', "input:not(.displayonly), button:not(.displayonly)", optionsForm.handleClick);
    $(document).on('keyup', "input:not(.displayonly), textarea:not(.displayonly)", optionsForm.handleKeyup);
    $(document).on('change', "select, input:not(.displayonly)", optionsForm.handleValueChange);
