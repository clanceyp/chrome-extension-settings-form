...
describe("OptionsForm API", function() {
	...
	describe("optionsForm.createRow.basic(element)", function() {
		...
		it("it creates a form element with HTML5 type", function() {
			...
			runs(function() {
				expect( document.querySelectorAll("input#foo-id[type='number']").length ).toBe( 1 );
			});
		});
		it("it creates a form element with data", function() {
			...
			runs(function() {
				expect( document.querySelectorAll("input#foo-id[data-foo]").length ).toBe( 1 );
				expect( document.querySelector("input#foo-id[data-foo]").getAttribute("data-foo") ).toBe( "bar" );
			});
		});
